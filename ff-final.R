...
#	tau1: means for each hidden state for binomial response (vector of length hs)
#	mi: number of 'days' for the Binomial response (fixed for all observation times, integer)
...
rx.=NULL,fitRx=c(FALSE,FALSE),w=list(matrix(c(1,0),nr=1),matrix(c(0,1),nr=1)))  {
	...
	for(i in 1:N) {
		...
		tmpneta1 <- sum(tau1[1:Z[i,1]]) + w[[1]]%*%Res[i,];
		tmpneta2 <- sum(tau2[1:Z[i,1]]) + w[[2]]%*%Res[i,];
		y1[i,1] <- rbinom(1,mi,exp(tmpneta1)/(1+exp(tmpneta1)));
		...
		for(j in 2:ni) {
			...
			tmpneta1 <- sum(tau1[1:Z[i,j]]) + w[[1]]%*%Res[i,];
			tmpneta2 <- sum(tau2[1:Z[i,j]]) + w[[2]]%*%Res[i,];
			y1[i,j] <- rbinom(1,mi,exp(tmpneta1)/(1+exp(tmpneta1)));
	...
	return(list(y1=y1,y2=y2,mi=matrix(rep(mi,N*ni),nr=N,nc=ni),N=N,ni=rep(ni,N),Z=Z,randomEffects=Res,rx=rx.,hs=hs,pars=mpars,fitRx=fitRx))
}
...
inccode <- 'List  HMMlabl(NumericVector y1, NumericVector y2, NumericVector states, NumericVector lmb, NumericMatrix p, NumericVector pi, NumericVector ni, NumericVector mi) {
//Code adapted from Zucchini and MacDonald (2009)
NumericVector x1(y1); NumericVector x2(y2);
NumericVector meep(mi);
...
for(int q=0; q<n; q++) {
	for(int r=0; r<m; r++) {
		allprobs(q,r) = ::Rf_dbinom(x1[q],meep[q],lambda[r],false)*::Rf_dnorm4(x2[q],lambda[r+m],lambda[2*m],false);
	}
...
IntegerVector hsseq =	seq_len(me);
	...
	for(int cf=0; cf<me; cf++) {
		lmbtmp[cf] = ::exp(basepois1[cf] +beta[re1pidx[i]-1])/(1+::exp(basepois1[cf] +beta[re1pidx[i]-1]));
	...
	List out = HMMlabl(tmpy1,tmpy2,me,lmbtmp,pii,piii,ni[i],meep.row(i));
...
pc.com <- cxxfunction(signature( y1m = "matrix", y2m = "matrix", betaa="numeric",m="integer",nis="integer",ccc="matrix",mi="matrix",rx1="integer",paridx="List",R_fitRx="logical"),code5,plugin = "RcppArmadillo",includes=c('#include <RcppArmadilloExtensions/sample.h>','#include <cmath>',inccode));
...
code <- 'Environment mvtnorm("package:mvtnorm");
	...
	for (int i=0; i<N; i++) {
		...
		for (int j=0; j<nii[i]; j++) {
				ocpb(j) = ::Rf_dbinom(y1(i,j),meep(i,j),::exp(ors(i,0) + fixedpp(i,j))/(1+::exp(ors(i,0) + fixedpp(i,j))),true);
				ncpb(j) = ::Rf_dbinom(y1(i,j),meep(i,j), ::exp(nrs(i,0) + fixedpp(i,j))/(1+::exp(nrs(i,0) + fixedpp(i,j))),true);
...
library(mvtnorm);
re.n <- cxxfunction(signature( y1m = "matrix", y2m = "matrix", zzz= 'matrix', ZZZ= 'matrix', b1 = "numeric",b2 = "numeric",nre = "matrix",ore="matrix",nis="numeric",mi="matrix",paridx="list",m="integer"),code,plugin = "RcppArmadillo");
...
# arguments:
# y1.: a vector of length N * ni of binomial responses (include NAs for missing data after dropout; this software will not currently handle intermittently missing data)
...
# mi.: a N x ni matrix of binomial "n"
...
# sig.y.1.: covariance matrix for MV normal proposal for MH sampler for binomial coefficients
...
# accp: # of accepted moves for MH sampler for binomial coefficients
# pidx: List of Parameter indices
#
###


simulated <- function(y1.,y2.,mi.,inits.,nsim.,report1.=1000,
...
hs=3,rx.=NULL,fitRx=c(FALSE,FALSE),scales.=c(0.01,0.01,0.01),id=rep(1:N.,6),hyperpar=c(3,1,1,.001,.0002),run.=1) {
	...
	scale2 <- scales.[2]; # For Binomial \taus
...
for(i in 2:nsim.) {
	...
	zz2 <- pc.com(y1m,y2m,o.betaa,hs,ni.,ccc,mi.,rx.,pidx,fitRx)
	zz2[zz2==0] <- NA;
	...
	mh.s <- re.n(y1m,y2m,zz2,ZZZ,o.betaa[1:basepars],n.betaa[1:basepars],new.res,old.res,ni.,mi.,pidx,hs);
	npp <- mh.s$npp;
	opp <- mh.s$opp;
	...
	npp <- sum(dbinom(y1.na,mi.na,exp(n.betaa[re1idx[id.na]] +ZZZ%*%t(new.betaa))/(1+exp(n.betaa[re1idx[id.na]] +ZZZ%*%t(new.betaa))),TRUE));
	if(is.na(npp)) {
		message(paste("Warning cannot sample from tau_1", o.betaa,sep=""));
		stop("");
	}
	 opp <- sum(dbinom(y1.na,mi.na,exp(n.betaa[re1idx[id.na]] +ZZZ%*%o.betaa[tau1idx])/(1+exp(n.betaa[re1idx[id.na]] +ZZZ%*%o.betaa[tau1idx])),TRUE));
	 u <- runif(1);
