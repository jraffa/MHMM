...
## estimates via a MCMC-based approach with simulated.pois()
...
file.name <- 'sim-hs3-1-pois'; #What you want R to save the posterior sample files to. (it will add the .Rdata extension)
source("ff-pois.R");
# Set seed
set.seed(25563);
...
x <- simulated.pois(y1.=as.numeric(daf$y1),y2.=as.numeric(daf$y2),ti.=daf$ti,inits.=c(daf$pars),nsim.=40000,ksamp.=40,
...
samp <- simulated.pois(y1.=as.numeric(daf$y1),y2.=as.numeric(daf$y2),ti.=daf$ti,inits.=colMeans(x$betaa[(ceiling(prelim.samps/fracc)+1):(prelim.samps),]),nsim.=3*10^6,ksamp.=60,
